...
ServerConfig LoadServerConfigFile(const std::string& path, std::string& warning) {
    ...
    while (std::getline(file, line)) {
        ...
        if (key == "bind_address") {
            ...
            config.simulationTickHz = std::max(config::policy::kMinPositiveRate, intValue);
        } else if (key == "snapshot_rate_hz" && ParseInt(value, intValue)) {
            config.snapshotRateHz = std::max(config::policy::kMinPositiveRate, intValue);
        } else if (key == "max_input_frames_per_second" && ParseInt(value, intValue)) {
            config.maxInputFramesPerSecond = std::max(config::policy::kMinPositiveRate, intValue);
        } else if (key == "max_chunk_hints_per_second" && ParseInt(value, intValue)) {
            config.maxChunkHintsPerSecond = std::max(config::policy::kMinPositiveRate, intValue);
        } else if (key == "max_chunk_resync_requests_per_second" && ParseInt(value, intValue)) {
            config.maxChunkResyncRequestsPerSecond = std::max(config::policy::kMinPositiveRate, intValue);
        } else if (key == "metrics_log_interval_seconds" && ParseInt(value, intValue)) {
            config.metricsLogIntervalSeconds = std::max(config::policy::kMinMetricsLogIntervalSeconds, intValue);
        } else if (key == "world_chunk_width_tiles" && ParseInt(value, intValue)) {
            config.worldConfig.chunkWidthTiles = std::max(config::policy::kMinWorldTileSize, intValue);
        } else if (key == "world_chunk_height_tiles" && ParseInt(value, intValue)) {
            config.worldConfig.chunkHeightTiles = std::max(config::policy::kMinWorldTileSize, intValue);
        } else if (key == "world_tile_size" && ParseInt(value, intValue)) {
            config.worldConfig.tileSize = std::max(config::policy::kMinWorldTileSize, intValue);
        } else if (key == "world_interest_radius_chunks" && ParseInt(value, intValue)) {
            config.worldConfig.interestRadiusChunks = std::max(config::policy::kMinWorldTileSize, intValue);
        } else if (key == "max_clients" && ParseInt(value, intValue)) {
            config.maxClients = std::max(config::policy::kMinPositiveRate, intValue);
            ...
            config.fakeLagMs = std::max(config::policy::kMinNetSimMs, floatValue);
        } else if (key == "fake_jitter_ms" && ParseFloat(value, floatValue)) {
            config.fakeJitterMs = std::max(config::policy::kMinNetSimMs, floatValue);
        } else if (key == "fake_loss_send_pct" && ParseFloat(value, floatValue)) {
            config.fakeLossSendPct =
                std::clamp(floatValue, config::policy::kMinPacketLossPct, config::policy::kMaxPacketLossPct);
        } else if (key == "fake_loss_recv_pct" && ParseFloat(value, floatValue)) {
            config.fakeLossRecvPct =
                std::clamp(floatValue, config::policy::kMinPacketLossPct, config::policy::kMaxPacketLossPct);
        } else {
    ...
    config.worldConfig.chunkWidthTiles = std::clamp(config.worldConfig.chunkWidthTiles, config::policy::kMinWorldTileSize,
                                                    config::policy::kMaxWorldChunkDimensionTiles);
    config.worldConfig.chunkHeightTiles =
        std::clamp(config.worldConfig.chunkHeightTiles, config::policy::kMinWorldTileSize,
                   config::policy::kMaxWorldChunkDimensionTiles);
    config.worldConfig.tileSize = std::max(config::policy::kMinWorldTileSize, config.worldConfig.tileSize);
    config.worldConfig.interestRadiusChunks =
        std::max(config::policy::kMinWorldTileSize, config.worldConfig.interestRadiusChunks);
    config.maxInputFramesPerSecond = std::max(config::policy::kMinPositiveRate, config.maxInputFramesPerSecond);
    config.maxChunkHintsPerSecond = std::max(config::policy::kMinPositiveRate, config.maxChunkHintsPerSecond);
    config.maxChunkResyncRequestsPerSecond =
        std::max(config::policy::kMinPositiveRate, config.maxChunkResyncRequestsPerSecond);
    config.metricsLogIntervalSeconds = std::clamp(config.metricsLogIntervalSeconds,
                                                  config::policy::kMinMetricsLogIntervalSeconds,
                                                  config::policy::kMaxMetricsLogIntervalSeconds);
